...
{
    /// <summary>
    /// Assert that objects are equal in value
    /// </summary>
    public static class ObjectAssert
    {
        ...
        internal static bool AreEqual(ObjectAssertionConfiguration configuration, object expected, object actual,
            out string message)
        {
            message = null;
            ...
            foreach (var p in expectedType.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                ...
                if (value1 == null || value2 == null)
                {
                    ...
                    {
                        message =
                            $"Not equals {p.DeclaringType?.Name}.{p.Name}, values: {value1 ?? "<null>"} - {value2 ?? "<null>"}";
                        return false;
                    }
                ...
                try
                {
                    ...
                    {
                        message =
                            $"Not equals {p.DeclaringType?.Name}.{p.Name} {Environment.NewLine}details: {message}";
                        return false;
                    }
                }
                ...
                {
                    message =
                        $"Not equals {p.DeclaringType?.Name}.{p.Name} {Environment.NewLine}details: {exception.Message}";
                    return false;
                }
            }
        ...
        private static bool AreEqual(ObjectAssertionConfiguration configuration, IDictionary expected,
            IDictionary actual, out string message)
        {
            ...
            foreach (var key in expected.Keys)
            {
                ...
                try
                {
                    ...
                    message =
                        $"Elements of dictionaries by {key} are not equal {Environment.NewLine}details: {message}";
                    return false;
                }
                catch (ObjectAssertionException exception)
                {
                    message =
                        $"Elements of dictionaries by {key} are not equal {Environment.NewLine}details: {exception.Message}";
                    return false;
                }
            }
        ...
        private static bool AreEqual(ObjectAssertionConfiguration configuration, IEnumerable expected,
            IEnumerable actual, out string message)
        {
            ...
            while (enum1.MoveNext())
            {
                ...
                try
                {
                    ...
                    {
                        message =
                            $"Elements of sequences by index [{index}] are not equal {Environment.NewLine}details: {message}";
                        return false;
                    }
                ...
                {
                    message =
                        $"Elements of sequences by index [{index}] are not equal {Environment.NewLine}details: {exception.Message}";
                    return false;
                }
            }
